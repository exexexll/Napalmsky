...
router.post('/user', requireAuth, (req, res) => {
    ...
    if (store_1.store.hasReportedUser(reporterUserId, reportedUserId)) {
    ...
    // Get user details
    const reportedUser = store_1.store.getUser(reportedUserId);
    const reporterUser = store_1.store.getUser(reporterUserId);
    ...
    store_1.store.createReport(report);
    // Check if user should be auto-banned (4+ unique reports)
    const reportCount = store_1.store.getReportCount(reportedUserId);
    console.log(`[Report] User ${reportedUser.name} now has ${reportCount} reports`);
    if (reportCount >= 4 && !store_1.store.isUserBanned(reportedUserId)) {
        // Auto-ban with temporary status pending review
        const userIps = store_1.store.getUserIps(reportedUserId);
        const banRecord = {
        ...
        store_1.store.createBanRecord(banRecord);
...
router.get('/pending', requireAuth, requireAdmin, (req, res) => {
    const pendingReviews = store_1.store.getPendingReviews();
    // Include full report details for each
    const reviewsWithReports = pendingReviews.map(banRecord => ({
        ...banRecord,
        reports: store_1.store.getReportsForUser(banRecord.userId),
    }));
    res.json({
...
router.get('/all', requireAuth, requireAdmin, (req, res) => {
    const allReports = store_1.store.getAllReports();
    res.json({
        reports: allReports,
 ...
 */
router.post('/review/:userId', requireAuth, requireAdmin, (req, res) => {
    ...
    const banRecord = store_1.store.getBanRecord(userId);
    if (!banRecord) {
        return res.status(404).json({ error: 'Ban record not found' });
    }
    store_1.store.updateBanStatus(userId, decision, reviewerId);
...
router.get('/blacklist', (req, res) => {
    const blacklistedUsers = store_1.store.getBlacklistedUsers();
 ...
 */
router.get('/check-ban', requireAuth, (req, res) => {
    const user = store_1.store.getUser(req.userId);
    if (!user) {
        return res.status(404).json({ error: 'User not found' });
    }
    const banRecord = store_1.store.getBanRecord(req.userId);
    res.json({
        isBanned: store_1.store.isUserBanned(req.userId),
...
router.get('/stats', requireAuth, requireAdmin, (req, res) => {
    const allReports = store_1.store.getAllReports();
    const allBans = store_1.store.getAllBanRecords();
    const pending = store_1.store.getPendingReviews();
    const blacklisted = store_1.store.getBlacklistedUsers();
    res.json({
