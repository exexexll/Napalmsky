      ...
      >
        ...
        <div className="rounded-xl bg-white/5 p-6 border border-white/10">
          <div className="flex items-center gap-4 mb-4">
            {targetUser.selfieUrl ? (
              <div className="relative h-20 w-20 flex-shrink-0 overflow-hidden rounded-full border-4 border-[#ff9b6b]/30">
                <Image
        ...
        <div className="space-y-3">
          {isTargetOnline ? (
            <>
              <button
                onClick={onCallNow}
                className="focus-ring w-full rounded-xl bg-[#ff9b6b] px-6 py-4 font-medium text-[#0a0a0c] text-lg shadow-sm transition-opacity hover:opacity-90"
              >
            ...
            <>
              <div className="rounded-xl bg-orange-500/10 px-6 py-4 border border-orange-500/30 text-center">
                ...
                <p className="mt-2 text-sm text-[#eaeaf0]/70">
                  You can find them later in matchmaking or use your intro code: <span className="font-mono text-[#ff9b6b]">{referralCode}</span>
                </p>
              </div>
            </>
          )}
        ...
        <div className="rounded-lg bg-white/5 p-4">
          <p className="text-xs text-[#eaeaf0]/50 leading-relaxed">
            💡 <strong>Tip:</strong> You can use your intro code <span className="font-mono text-[#ff9b6b]">{referralCode}</span> on the main page anytime to directly match with {targetUser.name}.
          </p>
        </div>
      </motion.div>
    </div>
